...
#The maximum divisibility, makes everything an invariant
MAX_DIVISIBILITY = 1
...
class Monkey:
    ...
    def adjust_worry_level(self, worry_level):
        ...
        #PART 2 CHANGE:
        #the rest relies on divisibility, so can just take mod of values, using product of all divisibilities
        global MAX_DIVISIBILITY
        worry_level = (worry_level % MAX_DIVISIBILITY)
        ###
        ###

        if self.operator == '+':
            return int(worry_level + adjustment_value)
        else:
            return int(worry_level * adjustment_value)
